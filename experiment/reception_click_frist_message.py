...
3.判断是否完成任务，如果完成，返回action为'terminate'，status为'success'，否则返回action为'terminate'，text为'任务未完成'。
按照如下格式返回动作：
<tool_call>
...
test_plan_action(r"C:\lenovo_sx\JD-RPA-pro\assert\reception_click_first_message_1.png",task_prompt)

test_plan_action(r"C:\lenovo_sx\JD-RPA-pro\assert\reception_click_first_message_success.png",task_prompt)
