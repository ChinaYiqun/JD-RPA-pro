...
def find_all_matches(image_path, template_paths, threshold=0.58, show=False, iou_threshold=0.3):
    """
...
# 将坐标和文本信息汇报绘制到图片上
def draw_text_on_image(text, coordinate, image_path, output_path=None):
    """
    在图片上标记指定坐标
    ...
    # 绘制标记 (红色十字)
    draw.line((x - 10, y, x + 10, y), fill="red", width=3)  # 水平线
    draw.line((x, y - 10, x, y + 10), fill="red", width=3)  # 垂直线
    # 绘制文本,文字要在红色十字上方
    draw.text((x, y - 10), text, fill="green")
...
def get_image_size_from_base64(base64_str):
    # 1. 去除可能的前缀（如 data:image/xxx;base64,）
...
def draw_point(image_path: str, point: list, color=None):
...
if __name__ == '__main__':
    for i in range(7):
        find_all_matches(f"resource/left_{i+1}.png", ["resource/new_message_button_1.png", "resource/new_message_button_2.png","resource/new_message_button_3.png"],show=True)

    find_all_matches("message_list.png",
                     ["resource/new_message_button_1.png", "resource/new_message_button_2.png",
                      "resource/new_message_button_3.png"], show=True)

# mark_coordinate_on_image((54+220, 171), r'resource/left_1.png',"resource/left_1_mark.png")
